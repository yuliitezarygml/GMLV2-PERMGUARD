...
# Import unified PermGuard module
from permguard_auth import permguard_auth, require_permission, check_permission, get_auth_stats, get_auth_logs, get_permguard_status, get_traffic_stats, get_traffic_logs
...
class SwaWebAuth:
    """Legacy compatibility wrapper for unified PermGuard module"""

    @staticmethod
    def check_permission(user_email, role, resource_type, action, resource_id=None, context=None, resource_properties=None, subject_properties=None):
        """Legacy method - redirects to unified PermGuard module"""
        # Convert to unified format
        action_unified = f"{resource_type}::{action}"
        resource_unified = f"{resource_type}::{resource_id}" if resource_id else f"{resource_type}::default"

        # Build context
        unified_context = context or {}
        if resource_properties:
            unified_context.update(resource_properties)
        if subject_properties:
            unified_context.update(subject_properties)
        unified_context['role'] = role

        # Use unified PermGuard module
        result = check_permission(user_email, action_unified, resource_unified, unified_context)

        # Convert back to legacy format
        return {
            'allowed': result['allowed'],
            'reasons': [result.get('reason', 'Access denied')] if not result['allowed'] else []
        }

# Legacy decorator for backward compatibility
def legacy_require_permission(resource_type, action, get_resource_data=None):
    """Legacy decorator - use new require_permission from permguard_auth instead"""
    def decorator(f):
        @wraps(f)
        def decorated_function(*args, **kwargs):
            ...
            user_email = session.get('username', 'unknown@swaweb.com')
            role = session.get('role', 'user')

            # Get resource data if function provided
            resource_data = get_resource_data(*args, **kwargs) if get_resource_data else {}

            # Use SwaWebAuth for legacy compatibility
            auth_result = SwaWebAuth.check_permission(
                user_email,
                role,
                resource_type,
                action,
                ...
                resource_data.get('resource_properties'),
                resource_data.get('subject_properties')
            )

            if not auth_result['allowed']:
                return jsonify({
                    'error': 'Permission denied',
                    'reasons': auth_result.get('reasons', ['Access not permitted'])
                }), 403
...
# ===== PERMGUARD ADMIN ROUTES =====

@app.route('/admin/permguard')
@admin_required
def admin_permguard_dashboard():
    """PermGuard admin dashboard"""
    try:
        # Get PermGuard status
        permguard_status = {
            'connected': az_client is not None,
            'endpoint': f"{PERMGUARD_ENDPOINT}:{PERMGUARD_PORT}",
            'workspace_id': WORKSPACE_ID,
            'policy_store_id': POLICY_STORE_ID
        }

        # Get authorization statistics from logs
        auth_stats = get_permguard_stats()

        # Get recent authorization requests
        recent_auths = get_recent_auth_requests()

        return render_template('admin/permguard.html',
                             permguard_status=permguard_status,
                             auth_stats=auth_stats,
                             recent_auths=recent_auths)
    except Exception as e:
        logger.error(f"PermGuard admin dashboard error: {e}")
        return render_template('admin/permguard.html',
                             error=str(e),
                             permguard_status={'connected': False})

@app.route('/admin/permguard/logs')
@admin_required
def admin_permguard_logs():
    """PermGuard authorization logs"""
    ...
    logs = get_permguard_logs(page=page, per_page=per_page,
                             user_filter=filter_user, result_filter=filter_result)

    return render_template('admin/permguard_logs.html',
                         logs=logs['logs'],
                         pagination=logs['pagination'],
                         filters={'user': filter_user, 'result': filter_result})

@app.route('/admin/permguard/test')
@admin_required
def admin_permguard_test():
    """PermGuard policy testing interface"""
    return render_template('admin/permguard_test.html')

@app.route('/api/admin/permguard/test', methods=['POST'])
@admin_required
def api_admin_permguard_test():
    """Test PermGuard authorization"""
    try:
        data = request.get_json()

        user_email = data.get('user_email', 'test@swaweb.com')
        role = data.get('role', 'user')
        resource_type = data.get('resource_type', 'SwaWeb::Game::Item')
        action = data.get('action', 'SwaWeb::Game::Action::view')
        ...
        result = SwaWebAuth.check_permission(
        ...
        # Log the test
        logger.info(f"Admin PermGuard test: {user_email} {action} -> {'ALLOW' if result['allowed'] else 'DENY'}")
        ...
        logger.error(f"PermGuard test error: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500

@app.route('/api/admin/permguard/stats')
@admin_required
def api_admin_permguard_stats():
    """Get PermGuard statistics API"""
    try:
        stats = get_permguard_stats()
        return jsonify({
            'success': True,
            'stats': stats
        })
...
def get_permguard_stats():
    """Get PermGuard authorization statistics"""
    try:
        # In a real implementation, these would come from a database or log analysis
        # For demo purposes, we'll simulate some statistics
        return {
            'total_requests': 156,
            'allowed_requests': 142,
            'denied_requests': 14,
            'success_rate': 91.0,
            'top_users': [
                {'user': 'user@swaweb.com', 'requests': 45, 'success_rate': 95.0},
                {'user': 'premium@swaweb.com', 'requests': 38, 'success_rate': 97.0},
                {'user': 'admin@swaweb.com', 'requests': 32, 'success_rate': 100.0}
            ],
            'top_actions': [
                {'action': 'SwaWeb::Game::Action::view', 'count': 89},
                {'action': 'SwaWeb::Purchase::Action::buy', 'count': 34},
                {'action': 'SwaWeb::Wishlist::Action::add', 'count': 23}
            ],
            'recent_trends': {
                'today': {'requests': 23, 'denied': 2},
                'yesterday': {'requests': 31, 'denied': 3},
                'week_ago': {'requests': 28, 'denied': 1}
            }
        }
    except Exception as e:
        logger.error(f"Error getting PermGuard stats: {e}")
        return {'error': str(e)}

def get_recent_auth_requests(limit=10):
    """Get recent authorization requests"""
    try:
        # In a real implementation, this would come from logs or database
        # For demo purposes, we'll simulate recent requests
        from datetime import timedelta

        recent_requests = []
        for i in range(limit):
            request_time = datetime.now() - timedelta(minutes=i*5)
            recent_requests.append({
                'timestamp': request_time.isoformat(),
                'user': ['user@swaweb.com', 'premium@swaweb.com', 'teen@swaweb.com'][i % 3],
                'action': ['view', 'purchase', 'wishlist'][i % 3],
                'resource': f'game-{(i%5)+1:03d}',
                'result': 'ALLOW' if i % 7 != 0 else 'DENY',
                'reason': 'Policy matched' if i % 7 != 0 else 'Insufficient permissions',
                'duration_ms': 15 + (i % 10)
            })

        return recent_requests
    except Exception as e:
        logger.error(f"Error getting recent auth requests: {e}")
        return []

def get_permguard_logs(page=1, per_page=50, user_filter='', result_filter=''):
    """Get PermGuard authorization logs with pagination"""
    try:
        ...
        all_logs = []
        for i in range(200):  # Simulate 200 log entries
            log_time = datetime.now() - timedelta(hours=i)
            user = ['user@swaweb.com', 'premium@swaweb.com', 'teen@swaweb.com', 'admin@swaweb.com'][i % 4]
            result = 'ALLOW' if i % 8 != 0 else 'DENY'

            # Apply filters
            if user_filter and user_filter.lower() not in user.lower():
                continue
            if result_filter and result_filter.upper() != result:
                continue
            ...
            all_logs.append({
                ...
                'resource_type': 'SwaWeb::Game::Item',
                'action': ['view', 'purchase', 'wishlist', 'download'][i % 4],
                'resource_id': f'game-{(i%10)+1:03d}',
                'result': result,
                'reason': 'Policy matched' if result == 'ALLOW' else ['Insufficient funds', 'Age restriction', 'Already owned'][i % 3],
                'duration_ms': 10 + (i % 20),
                'ip_address': f'192.168.1.{(i % 100) + 1}'
            })
        ...
        logger.error(f"Error getting PermGuard logs: {e}")
        return {
            'logs': [],
...
@app.route('/api/admin/traffic/stats')
@admin_required
def api_admin_traffic_stats():
...
@app.route('/api/admin/traffic/logs')
@admin_required
def api_admin_traffic_logs():
    """Get traffic logs"""
    try:
        ...
        if request.args.get('min_duration'):
            filters['min_duration'] = request.args.get('min_duration')

        logs = get_traffic_logs(limit, filters if filters else None)
        return jsonify(logs)
    except Exception as e:
        logger.error(f"Error getting traffic logs: {e}")
        return jsonify({'error': 'Failed to get traffic logs'}), 500

@app.route('/admin/traffic')
@admin_required
def admin_traffic():
    """Traffic monitoring admin page"""
    return render_template('admin/traffic.html')

@app.route('/api/admin/traffic/demo', methods=['POST'])
@admin_required
def api_admin_traffic_demo():
    """Initialize demo traffic data"""
    try:
        permguard_auth.add_demo_traffic_data()
        stats = get_traffic_stats()
        return jsonify({
            'success': True,
...
# ===== PERMGUARD GAMESTORE ROUTES =====

@app.route('/api/games/purchase/<game_id>', methods=['POST'])
@require_permission('game::purchase', lambda game_id: f'game::{game_id}')
def purchase_game(game_id):
    """Purchase a game with PermGuard authorization"""
    logger.info(f"Purchase attempt: user={session.get('username')}, game={game_id}")

    # Load current games data
    games_free = load_json_file('games_free_backup.json', [])
    games_premium = load_json_file('games_premium_backup.json', [])

    # Find the game
    game = None
    is_free = False

    # Check free games first
    for i, g in enumerate(games_free):
        if f'free-{i+1:03d}' == game_id:
            game = g
            is_free = True
            break

    # Check premium games if not found
    if not game:
        for i, g in enumerate(games_premium):
            if f'premium-{i+1:03d}' == game_id:
                game = g
                is_free = False
                break
    ...
    game_price = 0 if is_free else float(game.get('price', 19.99))
    user_balance = float(user_data.get('balance', 0))
    owned_games = user_data.get('owned_games', [])

    # Additional PermGuard context check
    context = {
        'alreadyOwned': game_id in owned_games,
    ...
    # Re-check permission with full context
    auth_result = SwaWebAuth.check_permission(
        username,
        session.get('role', 'user'),
        'SwaWeb::Game::Item',
        'SwaWeb::Purchase::Action::buy',
        game_id,
        context,
        resource_props,
        {
    ...
    if not auth_result['allowed']:
        return jsonify({
            'error': 'Cannot purchase this game',
            'reasons': auth_result.get('reasons', ['Access not permitted'])
        }), 403

    # Check if already owned
    if game_id in owned_games:
        return jsonify({'error': 'Game already owned'}), 400

    # Check balance for paid games
    if not is_free and user_balance < game_price:
    ...
    try:
        # Update user data
        if not is_free:
            user_data['balance'] = user_balance - game_price

        owned_games.append(game_id)
        user_data['owned_games'] = owned_games
        ...
        logger.info(f"Purchase successful: {game.get('name', 'Unknown')} by {username}")

        return jsonify({
            'message': f'Successfully purchased {game.get("name", "game")}',
        ...
        return jsonify({'error': 'Purchase processing failed'}), 500

@app.route('/api/games/library')
@login_required
def get_user_library():
    ...
    for game_id in owned_games:
        ...
        # Check free games
        if game_id.startswith('free-'):
            try:
                index = int(game_id.split('-')[1]) - 1
                if 0 <= index < len(games_free):
                    game_data = games_free[index]
                    game_data['id'] = game_id
                    game_data['isFree'] = True
            except:
                pass
        ...
        # Check premium games
            ...
            try:
                index = int(game_id.split('-')[1]) - 1
                if 0 <= index < len(games_premium):
                    game_data = games_premium[index]
                    game_data['id'] = game_id
                    game_data['isFree'] = False
            except:
                pass

        if game_data:
            library.append(game_data)

    return jsonify(library)

@app.route('/api/games/wishlist')
@login_required
def get_user_wishlist():
    ...
    # Convert wishlist IDs to game data (similar to library)
    games_free = load_json_file('games_free_backup.json', [])
    games_premium = load_json_file('games_premium_backup.json', [])
...
@app.route('/api/games/wishlist/<game_id>', methods=['POST'])
@login_required
def add_to_wishlist(game_id):
    ...
    # Check if already owned
    if game_id in user_data.get('owned_games', []):
        return jsonify({'error': 'Game already owned'}), 400

    # Add to wishlist
    wishlist = user_data.get('wishlist', [])
    if game_id not in wishlist:
...
@app.route('/api/games/wishlist/<game_id>', methods=['DELETE'])
@login_required
